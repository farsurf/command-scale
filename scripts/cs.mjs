...
function cmdReport() {
  ...
  console.log('');
  console.log('  Beside it, in whatever language they are speaking, say what it means:');
  console.log('  where they stand, which situation is lowest, and what the next height');
  console.log('  would close. Do not write the sentence that would close it.');
  console.log('');
  console.log('  The reading is also a page, and a page is handed over only when they');
  console.log('  can see it. Use whatever your own tooling can actually put in front of');
  console.log('  them, best first:');
  console.log('    · a surface of yours that displays a page, an image or a document —');
  console.log('      `node scripts/cs.mjs page` writes the page out for you to pass to it');
  console.log('    · a file where they are already looking —');
  console.log('      `node scripts/cs.mjs page --out <in the folder they have open>`');
  console.log(`    · failing both, say where it is: ${html}`);
  console.log('  That last one is last. A path is on the machine this ran on, which is');
  console.log('  often not the machine they are sitting at, and a path they cannot open');
  console.log('  is a reading they were not given.');
  console.log('');
