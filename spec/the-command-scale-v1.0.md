...
Ability is demonstrated in several occasions, and each of the first five levels is held only when every occasion that confers it holds at that height; the sixth is held as Appendix A.5 states. Three confer: **Request**, **Review**, **Roadblock**. A fourth, **Question**, is placed on a ladder of its own. The top of that ladder is part of what the sixth level requires; its first five levels begin to confer on a single date, the same for everyone assessed, one month after an implementation begins to arrange practice in it, and until then those five are recorded and confer nothing. Its definition, its levels and its admission are in Appendix A. This is not a formality. A person who makes good Requests, cannot say what is wrong with what came back, and says the same thing again when stopped, does not have the level: that person has one conferring situation and is missing the others, and the missing ones are where most of the practical loss happens.
...
**Question levels 1 to 5 confer after a month of practice.** From the day an implementation begins to arrange practice in this situation (§8), choosing it for practice exactly as it chooses the other three, including for people who rarely ask, Question levels 1 to 5 are practised, recorded, counted per level under the thresholds of §5.3 and shown, and confer nothing. One month later, Question level n begins to count toward level Ln, for n from 1 to 5, exactly as the other three situations do: from that date a level from L1 to L5 is not held unless this situation has been made good at the Question level of the same number. Both dates are the same for everyone the implementation assesses, and the implementation states both dates where it states its threshold values (§5.3). The month is part of this rule; a different period is a change to the normative text and appears in its revision history (Appendix B.7). Placements recorded before the second date count after it; nothing is read again. A level lost because this situation begins to count is published as a reduction caused by this admission, which it states in place of an error in an earlier reading (§4; Appendix B.4).
...
A situation confers a level when it is counted toward one. Request, Review and Roadblock confer. Question confers at the sixth level, which is held only together with the top of its ladder. At the first five levels it begins to confer one month after an implementation begins to arrange practice in it, on a single date for everyone it assesses; the implementation states both dates, and until the second it is placed and recorded and confers nothing. Its levels, the rules for placing it and the rule for its admission are in Appendix A of this paper. Any change to this is a change to this appendix and appears in B.7.
...
| Version | Date | Change |
| --- | --- | --- |
| 1.0 | — | First publication, not yet released. Four situations: Request, Review, Roadblock and **Question**; the levels of Question and its admission are in Appendix A of this paper. |
